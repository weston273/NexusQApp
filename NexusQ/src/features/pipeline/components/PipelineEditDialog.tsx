        ...
        <DialogContent className="sm:max-w-[780px] max-h-[84vh] overflow-y-auto">
          ...
          <DialogFooter className="gap-2 sm:justify-between">
            <div className="flex w-full justify-start">
              {canDeleteLead ? (
                <Button
                  ...
                  disabled={saving || deleting || !activeLead}
                >
                  {deleting ? "Deleting..." : "Delete Lead"}
                </Button>
              ) : (
                <span className="text-xs text-muted-foreground">Only workspace owners and admins can delete leads.</span>
              )}
            </div>
            <div className="flex w-full flex-col-reverse gap-2 sm:w-auto sm:flex-row">
              ...
              <Button onClick={onSave} disabled={saving || deleting || !activeLead || missingRequiredValue}>
      ...
      <AlertDialog open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {activeLead?.name ?? "This lead"} will be removed from the workspace database. Pipeline state, stored
              messages, lead events, and lead-linked notifications for this record will also be deleted.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleting}>Keep Lead</AlertDialogCancel>
            <AlertDialogAction
