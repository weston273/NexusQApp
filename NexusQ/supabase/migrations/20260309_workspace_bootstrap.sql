-- Workspace bootstrap and secure join helpers used by edge functions.
-- These functions are intentionally service-role only to keep critical
-- tenant onboarding logic off the browser and outside RLS edge cases.

create extension if not exists pgcrypto;

create or replace function public.pick_higher_access_role(
...
as $$
declare
  v_existing text := lower(coalesce(p_existing_role, 'viewer'));
  v_new text := lower(coalesce(p_new_role, 'viewer'));
begin
  if v_existing not in ('owner', 'admin', 'viewer') then
    v_existing := 'viewer';
  end if;

  if v_new not in ('owner', 'admin', 'viewer') then
    v_new := 'viewer';
  end if;

  if v_existing = 'owner' or v_new = 'owner' then
    return 'owner';
  end if;

  if v_existing = 'admin' or v_new = 'admin' then
    return 'admin';
  end if;
...
begin
  ...
  return query
  select v_client_id, v_final_role;
end;
$$;
